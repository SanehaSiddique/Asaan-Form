...
try:
    ...
    fitz = None
from app.services.ocr_service import extract_english_text, load_input, run_ocr, group_lines
    ...
    async def extract_text(
        ...
        if file_ext == ".pdf":
            ...
            if self.use_docling_for_pdfs:
                try:
                    print(f"  Using Docling for PDF OCR (faster)...")
                    ...
                    if "urdu" in languages:
                        print(f"  ⚠️ Urdu OCR not available for PDFs via Docling")
                        result["urdu_text"] = ""
                    ...
                    print(f"  ⚠️ Docling failed ({str(docling_error)[:100]}), falling back to image-based OCR...")
                        ...
                        if "english" in languages:
                            try:
                                ...
                                if "timeout" in error_msg.lower() or "hang" in error_msg.lower():
                                    print(f"    ⚠️ OCR appears to be hanging, skipping remaining pages...")
                                    break
